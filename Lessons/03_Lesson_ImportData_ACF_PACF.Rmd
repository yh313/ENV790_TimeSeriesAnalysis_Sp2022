...
```
```{r}
#adv of relative path: if from other's file can still open it
raw_inflow_data<-read.table(file="./Data/inflowtimeseries.txt.",head=FALSE,skip=0)
```
...
raw_inflow_data <- read.table(file="C:/Users/lenovo/Desktop/Spring_2022/ENV790/ENV790_TimeSeriesAnalysis_Sp2022/Data/inflowtimeseries.txt.",header=FALSE, skip=0)
...
```{r}
format(my_date[1],format="%y-%m")
format(my_date[1],format="%Y-%m")#want four digits for year
format(my_date[1],format="%Y-%b")
```
...
#can enter all the command so don't need to console instead of inline
