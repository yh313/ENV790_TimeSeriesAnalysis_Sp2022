...
```{r}
#Using R decompose function, has to be on ts data
iHP=1
...
cat(mean_inflow,sd_inflow)
#after removing trend and seasonality, mean reaching =0
...
#as.factor making variable into categorical data
...
```
Data is ready! (ready to check the outliers)

## Visualizing outliers in R

```{r}
#Generating a box plot by factor where factor is month of the year
#use data frame in ggplot,geo_line/geo_spot
#still have the randomness of the wet months, seasonality just remove the mean of it.
...
#not okay to use um fill=TRUE on original time series data
...
while(pvalue < 0.05){ #the algorithm only enter the loop if the p-value 
  ...
  out_test <- grubbs.test(aux_inflow,type=10)#type 10 is a test for one outlier.
