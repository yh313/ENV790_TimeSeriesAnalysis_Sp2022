...
wind_season <- decompose_wind_data$seasonal  
wind_trend <- decompose_wind_data$trend

#Ex3: Create one data frame with all three vector and the Date columns from wind_data

Composition_wind_data <- data.frame(date=full_wind_data$Date, Trend=wind_trend,Seasonal=wind_season,Random=wind_random)

#Ex4: Inspect the data frame using head() and tail()
head(Composition_wind_data,10)
tail(Composition_wind_data,10)
...
Is the way that it is detrending and seasonality, 
## Coding challenge
```{r}

#Ex5: Try to reproduce the trend component from decompose().
order<-6, 
#why decompose: deterministic, and stochastic, and have multiple frequency, better tool than linear deterministic trend.
