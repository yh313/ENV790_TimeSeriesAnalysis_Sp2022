...
```{r}
#Create line graphs with multiple variables
#ggplot(RE_data, aes(x=Month)) + geom_line(aes(y=RE_data$`Total Renewable Energy Production`)) +xlab("Year") + geom_line(aes(y=RE_data$`Total Biomass Energy Production`,col="Biomass"))+ geom_line(aes(y=RE_data$`Total Renewable Energy Production`,col="Renewables")) +geom_line(aes(y=RE_data$`Hydroelectric Power Consumption`,col="Hydro"))+ xlab("Year") + ylab("Total Energy Production (Trillion Btu)")+ labs(color="Types") + theme_minimal()+ geom_hline(yintercept = mean(Indcatotvalue), color="blue")
```
```{r}
