...
> Answer: The value of PACF at lag 1 for the generated series matches the 0.6 coefficient for the autoregressive component. The order also matches the ACF at lag 1 because the order is 1. Smaller sample, harder to observe the match. for MA model, ACF, previous observations, but no information on previous error, so can't tell the coefficient of MA from ACF or PACF. 
...
#The seasonal moving average coefficient is -0.25
```
...
Pacf(sarima_model)
```
Cutoff at ACF at lag=1, MA (0,1)
multiple spikes in ACF at seasonal lag, but single spike in PACF, so has seasonal autoregression