...
plot(diff_RE_1,type="l",ylab="Differenced Total Renewable Energy Porduction (trillion Btu)",xlab="Time")
```
...
ggplot(New_renew, aes(x=Month, y=New_renew[,2],show.legend=TRUE)) +
  ...
  scale_color_manual(name = "Series", breaks=c("Original","Detrended (lm method)","Differenced"),
...
The stochastic trend indicated by the results matches what we observed in Q2 and Q3, where the plot shows that using linear regression coefficients to remove the trend is not an appropriate method, rather, differencing will eliminate the trend component better(less variation around zero).
### Q7
