...
#RE_data$Month<- ym(RE_data$Month)
diff_RE_1=diff(x=RE_data$`Total Renewable Energy Production`,lag=1,differences=1)
plot(diff_RE_1,type="l",ylab="Differenced Total Renewable Energy Porduction")
```
...
#library(lubridate)
#RE_data$Month <- my(RE_data$Month)
head(RE_data)
RE_data<-RE_data[-c(1),]
#RE_data$Month <-as.character.Date(RE_data$Month)
...
```{r}
#Use ggplot
ggplot(New_renew, aes(x=Month, y=New_renew[,2])) +
            geom_line(color="blue") +
            ylab("Total Renewable Energy Production") +
            #geom_smooth(color="red",method="lm") +
            geom_line(aes(y=New_renew[,3]), col="green")+
            geom_line(aes(y=New_renew[,4]), col="yellow")+
            scale_fill_discrete(labels=c("Original", "Detrended", "Defferenced"))
...
```{r}
#"Do they match what you observed in Q2?" Isn't Q2 the detrended data? 
#It seems like other than seasonality, there is still a great variation around zero, so it is consistent with the restuls from the two tests. 
ts_RE_data <- ts(RE_data[,3], start=c(1973, 1), end=c(2021, 09), frequency=12)

SMKtest <- SeasonalMannKendall(ts_RE_data)
print("Results for Seasonal Mann Kendall /n")
print(summary(SMKtest))
```
The seasonal Mann Kendall test indicates there is a trnend (p<0.05). tau is a positive number, which means it is an increasing trend.
...
#Group data in yearly steps instances, want second observation at second row first column, 
RE_data_matrix <- matrix(ts_RE_data,byrow=FALSE,nrow=12)
#take the mean of each column and store them in another vector
...
```
The result of Mann Kendall test indicates that there is a trend in the time series (p<0.05). The trend is positive as the tau is a positive number.
```{r}
...
print(sp_rho)
```
The Spearman correlation indicates that there is a trend in the aggregated series (p<0.05).
```{r}
...
The results from the test in agreement with the test results for the non-aggregated series.
```{r}
head(ts_RE_data)
```
